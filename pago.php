...
//Si se ha pulsado el botón pagar
if(filter_has_var(INPUT_POST, "Pagar")){
    //Recolectamos los datos introducidos en el formulario
    $fecha = filter_input(INPUT_POST, 'fecha');
    $concepto = filter_input(INPUT_POST, 'concepto');
    $cantidad = filter_input(INPUT_POST, 'cantidad');
    
    //Llamamos al metodo correspondiente, pasandole por parametros los datos
    $resultado = Operativa::pago($fecha, $concepto, $cantidad);
    
    //Si el resultado es positivo, es que existen errores que subsanar, por lo que
    //se lanzan los errores y se dejan rellenas los campos input
    if ($resultado) {
    ...
    //En el caso de no existir problemas, se lanza mensaje de info    
        ...
        $template->assign('info', '<p class="perfect">Se ha efectuado el pago correctamente, será redirigido a movimientos.</p>');
